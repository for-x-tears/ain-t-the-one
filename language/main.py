...
def test_language_detection(file_path: str):
    """
    Основная функция для тестирования модели определения языка из CSV-файла.
    ...
    try:
        # Указываем `lineterminator` для корректного чтения CSV, где могут быть переносы строк
    ...
    # Убедимся, что колонки имеют правильный тип данных
    ...
    # Модель возвращает словарь, из которого нам нужен только 'label'
    ...
    # Получаем уникальные метки из "правильных" ответов, чтобы отчет был полным
    ...
    # Используем .to_string() для полного вывода в консоль
